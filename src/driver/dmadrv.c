...
typedef struct
{
  ...
  DmaMode_t         mode;
#endif
  /// address values of the initial descriptor ( driver overload)
  uint32_t ui32AdrsSrcInitialDescr;
  uint32_t ui32AdrsDstInitialDescr;
} ChTable_t;
#endif
...
const LDMA_TransferCfg_t xferCfgloop = LDMA_TRANSFER_CFG_PERIPHERAL_LOOP( 0, 1 );
...
static Ecode_t StartTransferLoop( DmaMode_t             mode,
                              DmaDirection_t        direction,
                              unsigned int          channelId,
                              DMADRV_PeripheralSignal_t
                                                    peripheralSignal,
...
/// ( driver overload)
static void DMADRV_InitPRS(uint8_t ui8Chl,  uint32_t ui32AdrsSRC, uint32_t ui32AdrsDST);
static LDMA_Descriptor_t *  DMADRV_pDescLoop;
static LDMA_TransferCfg_t *  DMADRV_pTransfer;
...
Ecode_t DMADRV_Init( void )
{
  ...
  for ( i=0; i < EMDRV_DMADRV_DMA_CH_COUNT; i++ )
  {
    ...
    /// ( driver overload)
	chTable[ i ].ui32AdrsSrcInitialDescr = DMADRV_DEFAULT_DESCRIPTOR_ADRS_VALUE;
	chTable[ i ].ui32AdrsDstInitialDescr = DMADRV_DEFAULT_DESCRIPTOR_ADRS_VALUE;
  }
...
/***************************************************************************//**
 * @brief
 *  Initialize DMADRV to be triggered by the PRS SREQ 0.
 *
 * @details
 *  The DMA hw is initialized.
 *
 * @param[in] ui8Chl
 *  The channel Id to initialize.
 *
 * @param[in] ui32AdrsSRC
 *  The source address of the initial descriptor to set.
 *
 * @param[in] ui32AdrsDST
 *  The destination address of the initial descriptor to set.
 *
 * @return
 *  none
 ******************************************************************************/
void DMADRV_InitPRS(uint8_t ui8Chl, uint32_t ui32AdrsSRC, uint32_t ui32AdrsDST)
{
  	/* mask of the dma channel*/
    uint8_t ui8ChlMsk = 1 << ui8Chl;

    /* Init the dma structure */
    LDMA_Init_t sInitDma;
	sInitDma.ldmaInitCtrlNumFixed     = _LDMA_CTRL_NUMFIXED_DEFAULT; 	/* fixed priority mode, channel 0 has the highest priority */
	sInitDma.ldmaInitCtrlSyncPrsClrEn = ui8ChlMsk;                   	/* PRS Synctrig clear enable*/
	sInitDma.ldmaInitCtrlSyncPrsSetEn = ui8ChlMsk;                   	/* PRS Synctrig set enable. */
	sInitDma.ldmaInitIrqPriority      = EMDRV_DMADRV_DMA_IRQ_PRIORITY;	/* IRQ priority level */
	...
	//LDMA->LINKLOAD = ui8ChlMsk; /* to initate loading and execution of the first descriptor */

	/* enable peripheral requests for the corresponding channel */
	LDMA->REQDIS &= ~ui8ChlMsk;

	/* enable channel 0 interrupt */
	LDMA->IEN |= ui8ChlMsk;

	LDMA->CH[0].REQSEL = (LDMA->CH[0].CTRL & _LDMA_CH_REQSEL_MASK)
			| LDMA_CH_REQSEL_SOURCESEL_PRS 		/* which peripheral request signal connects to each DMA channel */
			| LDMA_CH_REQSEL_SIGSEL_PRSREQ0; 	/* Single Request SREQ */

	LDMA->CH[0].CFG = (LDMA->CH[0].CFG & _LDMA_CH_CFG_MASK)
			| LDMA_CH_CFG_SRCINCSIGN_DEFAULT 	/* not incremented */
			| LDMA_CH_CFG_DSTINCSIGN_POSITIVE;	/* incremented */
  	  	  	/* ARBSLOT only applies to channels marked as round robin */

	LDMA->CH[0].LOOP = (LDMA->CH[0].LOOP & _LDMA_CH_LOOP_MASK)
		| (7<<_LDMA_CH_LOOP_LOOPCNT_SHIFT); /* number of repetitions before enabling the transfer. */

	LDMA->CH[0].CTRL = LDMA_CH_CTRL_DSTMODE_ABSOLUTE 	/* ignored for the initial register descriptor (always = absolute) */
					| LDMA_CH_CTRL_SRCMODE_ABSOLUTE 	/* ignored for the initial descriptor (always = absolute)*/
					| LDMA_CH_CTRL_DSTINC_ONE			/* address incremented of 1 word (SIZE) between transfers */
					| LDMA_CH_CTRL_SIZE_WORD			/* smallest unit of data transfered */
					| LDMA_CH_CTRL_SRCINC_NONE			/* address not incremented between transfers */
					| LDMA_CH_CTRL_IGNORESREQ_DEFAULT	/* not used (cause the LDMA to ignore SREQ's and wait for a full Request (REQ) signal, FIFO full) */
					| (1<<_LDMA_CH_CTRL_DECLOOPCNT_SHIFT) 	/* not used (if set, decrement the LOOPCNT field in the LDMA_CHx_LOOP) */
					| LDMA_CH_CTRL_REQMODE_ALL			/* one request transfers all data */
					| LDMA_CH_CTRL_DONEIFSEN_DEFAULT  	/* not used (to generate interrupt on completion of descriptor or to link next descr if LINK bit set) */
					| LDMA_CH_CTRL_BLOCKSIZE_ALL 		/* amount of data transferred in one arbitration */
														/* how many DMA transfers occur before it re-arbitrates */
					| LDMA_CH_CTRL_BYTESWAP_DEFAULT		/* endianess not reversed */
					| (0<<_LDMA_CH_CTRL_XFERCNT_SHIFT)	/* (xfercnt+1)=1 DMA transfer that need to be done */
					| LDMA_CH_CTRL_STRUCTREQ_DEFAULT	/* not used (begin a transfer immediately after a new descriptor is loaded) */
					| LDMA_CH_CTRL_STRUCTTYPE_TRANSFER; /* DMA "transfer" structure type selected */

	LDMA->CH[0].SRC = ui32AdrsSRC;
	LDMA->CH[0].DST = ui32AdrsDST;

	uint32_t adr = 0;//&(LDMA->CH[0].CTRL);
	LDMA->CH[0].LINK = (LDMA->CH[0].LINK & _LDMA_CH_LINK_MASK)
	 	| (/*adrsDescriptor*/adr << _LDMA_CH_LINK_LINKADDR_SHIFT) /* 0: point to ourself */
		| (1 << _LDMA_CH_LINK_LINK_SHIFT) /* stop after loop */
		| (1 << _LDMA_CH_LINK_LINKMODE_SHIFT); /* relative link address */
}
 ...
 *  none
 ******************************************************************************/
void DMADRV_SetUpChannelForPRS(uint8_t ui8Chl,  uint32_t * pui32AdrsSRC, uint32_t * pui32AdrsDST)
{
	/* Init the dma register to trig a dma transfer on prs single request */
	DMADRV_InitPRS(ui8Chl,(uint32_t)pui32AdrsSRC, (uint32_t)pui32AdrsDST);

	/* Updates the mem addresses of the initial descriptor */
	chTable[ ui8Chl ].ui32AdrsSrcInitialDescr = LDMA->CH[0].SRC;
	chTable[ ui8Chl ].ui32AdrsDstInitialDescr = LDMA->CH[0].DST;

	/* Allocate the software channel */
	DMADRV_AllocateChannel((unsigned int*)&ui8Chl,NULL);
}
 ...
 *  none
 ******************************************************************************/
void DMADRV_CloseChannelForPRS(uint8_t ui8Chl)
{
	/* Updates the mem addresses of the initial descriptor */
	chTable[ ui8Chl ].ui32AdrsSrcInitialDescr = DMADRV_DEFAULT_DESCRIPTOR_ADRS_VALUE;
	chTable[ ui8Chl ].ui32AdrsDstInitialDescr = DMADRV_DEFAULT_DESCRIPTOR_ADRS_VALUE;

	/* Free the software channel */
	DMADRV_FreeChannel(ui8Chl);
}
 ...
 *   DMADRV @ref Ecode_t is returned.
 ******************************************************************************/
Ecode_t DMADRV_MemoryPeripheralLoop( unsigned int          channelId,
                                 DMADRV_PeripheralSignal_t
                                                       peripheralSignal,
                                 void                  *dst,
                                 void                  *src,
                                 ...
                                 void                  *cbUserParam )
{
  return StartTransferLoop( dmaModeBasic,
                        dmaDirectionMemToPeripheral,
                        channelId,
                        peripheralSignal,
                        dst,
 ...
 *   DMADRV @ref Ecode_t is returned.
 ******************************************************************************/
Ecode_t DMADRV_PeripheralMemoryLoop( unsigned int          channelId,
                                 DMADRV_PeripheralSignal_t
                                                       peripheralSignal,
                                 void                  *dst,
                                 void                  *src,
...
void LDMA_IRQHandler( void )
{
  ...
  bool bEnableChannel = false; /// ( overload driver)
...
#if 1 /// ( overload driver)

      if((chTable[ chnum ].allocated == true) && (chTable[ chnum ].ui32AdrsDstInitialDescr != DMADRV_DEFAULT_DESCRIPTOR_ADRS_VALUE)
    		  && (chTable[ chnum ].ui32AdrsSrcInitialDescr != DMADRV_DEFAULT_DESCRIPTOR_ADRS_VALUE))
      {
    	  bEnableChannel = true;
          /* ring buffer: if [ (current address of the initial descriptor) > (first address + size buffer) ] */
          if (LDMA->CH[0].DST >= (chTable[ chnum ].ui32AdrsDstInitialDescr + (DMADRV_LENGTH_DST_BUFFER * DMADRV_SIZE_SAMPLE_IN_OCTET)))
          {
        	  LDMA->CH[0].DST = chTable[ chnum ].ui32AdrsDstInitialDescr;
        	  uint32_t * p = LDMA->CH[0].SRC; // TODO: test to removed
        	  (*p)--;
          }
      }

#endif /// ( overload driver)
      ch = &chTable[ chnum ];
      if ( ch->callback != NULL )
      {
        ch->callbackCount++;
                              ...
                              void                  *cbUserParam )
{
  ...
  xfer.ldmaReqSel    = peripheralSignal;
  xfer.ldmaLoopCnt    = ui16nbloop;
  desc->xfer.xferCnt = len - 1;
  desc->xfer.decLoopCnt = 1;
  desc->xfer.dstAddr = (uint32_t)buf0;
  desc->xfer.srcAddr = (uint32_t)buf1;
  desc->xfer.size    = size;

  desc->xfer.linkMode = 1;
  desc->xfer.link     = 0;
