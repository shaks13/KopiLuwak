...
#else
// Assume 32kHz RTC/RTCC clock, cmuClkDiv_8 prescaler, 1 ticks per second
#define RTC_DIVIDER                     ( cmuClkDiv_32768 )
#endif
...
static uint8_t			  rtcc_ui8CalendarEvent = SRVRTCC_CALENDAREVENT_STARTMEAS;
#if (APP_CHOSEN_FLAG == APP_GLUEPOT)
static uint8_t			  rtcc_ui8HourEvent = SRVRTCC_CALENDAREVENT_HOUR_TICK;
#endif
...
{
  true,                 /* Start counting when init completed. */
  true,                /* Disable updating RTC during debug halt. */
...
/* notifies the channel of the capture compare event
 * -1: 		No event
 * 0,1,2: 	the channel number
 */
volatile int8_t RTCDRV_i8NbCCxChannelEvent;
...
Ecode_t RTCDRV_Init( void )
{
  ...
  RTCC_ChannelInit( 0, &initRTCCCompareChannel );
  RTCC_ChannelInit( 1, &initRTCCCompareChannel );
  RTCC_ChannelInit( 2, &initRTCCCompareChannel );
#endif
...
#endif
{
	...
	Kernel_QueueItem_struct sQueueItemCalendarEvent = {KERNEL_CREATE_RECANDSEND (KERNEL_SENSORTASKID,KERNEL_SENSORTASKID),
											KERNEL_MESSAGEID_CALENDAR_EVENT,
											1,NULL};
#if (APP_CHOSEN_FLAG == APP_GLUEPOT)
	Kernel_QueueItem_struct sQueueItemMinInterrupt; /* TODO: minute for the test, to change in hour */
#endif
...
#if (GLUEPOT_DEBUG == 0)
	/* Minute tick */
	if ( flags & KERNEL_RTCC_IF_LIFE_CALCULATION_TICK ) /* TODO: minute for the test, to change in hour */
	{
		inTimerIRQ = true;
		RTC_INTDISABLE( KERNEL_RTCC_IEN_LIFE_CALCULATION_TICK );
		RTC_INTCLEAR ( KERNEL_RTCC_IF_LIFE_CALCULATION_TICK );

		sQueueItemMinInterrupt.pData 				= &rtcc_ui8HourEvent;
		sQueueItemMinInterrupt.ui16NbByte 			= 1;
		sQueueItemMinInterrupt.ui16notification 	= KERNEL_MESSAGEID_CALENDAR_EVENT;
		sQueueItemMinInterrupt.urecvsender 		= KERNEL_CREATE_RECANDSEND (KERNEL_KERNELTASKID,KERNEL_KERNELTASKID);

		xQueueSendFromISR (sKernel_QueuePointer.pKernelQueue,&sQueueItemMinInterrupt, &xHigherPriorityTaskWoken);

		RTC_INTENABLE( KERNEL_RTCC_IEN_LIFE_CALCULATION_TICK );
	}
#else
	if (flags & KERNEL_RTCC_IF_LIFE_CALCULATION_TICK)
	{
		inTimerIRQ = true;

		if (((RTCC->IEN & KERNEL_RTCC_IEN_MASK_LIFE_CALCULATION_TICK) >> KERNEL_RTCC_IEN_SHIFT_LIFE_CALCULATION_TICK ) == 1)
		{
			RTC_INTDISABLE( KERNEL_RTCC_IEN_LIFE_CALCULATION_TICK );
			RTC_INTCLEAR ( KERNEL_RTCC_IF_LIFE_CALCULATION_TICK );
	...
	if ( flags & RTC_COMP_INT )
	{
		...
		/* TODO: (Several sensors purpose) may be used a mask for RTCDRV_i8NbCCxChannelEvent in
		 * the case where several CCx irq are triggered at the same time */
	...
	{
		RTC_INTCLEAR ( RTCC_IFC_CC0 );
		rtcc_ui8CalendarEvent = SRVRTCC_CALENDAREVENT_STARTMEAS;
		sQueueItemCalendarEvent.pData = &rtcc_ui8CalendarEvent;
		xQueueSendFromISR (sKernel_QueuePointer.pSensorQueue,&sQueueItemCalendarEvent, &xHigherPriorityTaskWoken);
	}

	if ( flags & RTCC_IF_CC2 )
	{
		RTC_INTCLEAR ( RTCC_IFC_CC2 );
		rtcc_ui8CalendarEvent = SRVRTCC_CALENDAREVENT_STOPMEAS;
		sQueueItemCalendarEvent.pData = &rtcc_ui8CalendarEvent;
		xQueueSendFromISR (sKernel_QueuePointer.pSensorQueue,&sQueueItemCalendarEvent, &xHigherPriorityTaskWoken);
	}

	if ( flags & RTCC_IF_DAYTICK )
	{
		RTC_INTCLEAR (RTCC_IFC_DAYTICK); /* clear the IRQ*/
		srvrtcc_IncreaseDayCounter ();
	}
